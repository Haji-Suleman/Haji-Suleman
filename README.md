# Hi there 👋, I’m **Haji Suleman**

AI / ML-focused Developer with strong backend skills and a disciplined math foundation.  
I build scalable backend systems and integrate ML concepts into real-world applications.

---

## About Me

- 🎯 Focused on **Machine Learning, AI & backend design**  
- 🛠️ Backend: Node.js, NestJS, REST APIs  
- 🧠 Strengthening **math fundamentals for ML**  
- 💡 Curious about **security, data systems & intelligent apps**

---

## What I’m Doing

- Backend-centric full-stack solutions  
- Integrating **ML features** into apps  
- Designing maintainable APIs & backend logic  
- Converting **math concepts into ML implementations**

---

## Tech Stack

**ML/AI:** Python, NumPy, PyTorch (in progress)  
**Backend:** Node.js, Express.js, NestJS, REST APIs  
**Databases:** MongoDB, MySQL  
**Frontend:** React.js, HTML, CSS, Tailwind  
**Languages:** JS, TS, Python, C++  

---

## GitHub Stats & Activity

![Profile Views](https://komarev.com/ghpvc/?username=Haji-Suleman&color=blue&style=flat)  
![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Haji-Suleman&show_icons=true&theme=radical)  
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Haji-Suleman&layout=compact&theme=radical)  
![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Haji-Suleman&theme=radical)  
![Activity Graph](https://activity-graph.herokuapp.com/graph?username=Haji-Suleman&theme=react-dark)

---

## Featured Projects

- [ML Classification](https://github.com/Haji-Suleman/Pytorch) ![Stars](https://img.shields.io/github/stars/Haji-Suleman/Pytorch?style=social)  
- [Doctor-Patient Web App](https://github.com/Haji-Suleman/Medical-and-Patient) ![Stars](https://img.shields.io/github/stars/Haji-Suleman/Medical-and-Patient?style=social)  
- [Full-Stack Food eCommerce](https://github.com/Haji-Suleman/Food-Delivery) ![Stars](https://img.shields.io/github/stars/Haji-Suleman/Food-Delivery?style=social)  

---

## Key Reads

**ML:** *Hands-On Machine Learning with Scikit-Learn, Keras & TensorFlow*  
**Math:** SAT Math guides — Kolby, McClenath, Weinfeld, Johnson  

---

## 📫 Connect With Me

- [GitHub](https://github.com/Haji-Suleman)  
- [LinkedIn](https://www.linkedin.com/in/haji-suleman-553753369/)  
- Email: mirhajisulemanjamali@gmail.com  
- Portfolio: [Your Portfolio](https://your-portfolio-link.com)  

---

⚡ I focus on **math, fundamentals & durable engi**
